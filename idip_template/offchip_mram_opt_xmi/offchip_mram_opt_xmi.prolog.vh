...
//
localparam I_PROFILER_HAS_PROFILER = (`NUM_OFFCHIP_MRAM_PROFILER!=0);
localparam I_PROFILER_NUM_PROFILER = (`NUM_OFFCHIP_MRAM_PROFILER==0)? 1 : `NUM_OFFCHIP_MRAM_PROFILER;
localparam I_PROFILER_BW_COUNTER = `BW_OFFCHIP_MRAM_PROFILER;

localparam I_PROFILER_BW_OUTPUT = BW_OUTPUT;
localparam I_PROFILER_BW_ADDR = BW_CELL_INDEX;
localparam I_PROFILER_BW_DATA = BW_DATA;

wire i_profiler_inst_rready;
wire [`OFFCHIP_MRAM_MMIOX_INST_BW_OPCODE-1:0] i_profiler_inst_opcode;
wire [`OFFCHIP_MRAM_MMIOX_INST_BW_OPERAND-1:0] i_profiler_inst_operand;
wire i_profiler_inst_rrequest;
wire [I_PROFILER_BW_ADDR-1:0] i_profiler_cell_addr;
wire i_profiler_cell_read;
wire [I_PROFILER_BW_DATA-1:0] i_profiler_cell_rdata;
wire i_profiler_cell_write;
wire [I_PROFILER_BW_DATA-1:0] i_profiler_cell_wdata;
wire i_profiler_output_wready;
wire i_profiler_output_wrequest;
wire [I_PROFILER_BW_OUTPUT-1:0] i_profiler_output_wdata;
...
assign control_rmx_inst_fifo_rrequest = i_profiler_inst_rrequest | i_cache_inst_rrequest;
assign control_rmx_operation_finish = control_rmx_inst_fifo_rrequest;

//
ERVP_OFFCHIP_MRAM_PROFILER
#(
  .BW_OUTPUT(I_PROFILER_BW_OUTPUT),
  .BW_ADDR(I_PROFILER_BW_ADDR),
  .BW_DATA(I_PROFILER_BW_DATA)
)
i_profiler
(
	.clk(clk),
  ...
  .inst_rready(i_profiler_inst_rready),
  .inst_opcode(i_profiler_inst_opcode),
  .inst_operand(i_profiler_inst_operand),
  .inst_rrequest(i_profiler_inst_rrequest),
  .cell_addr(i_profiler_cell_addr),
  .cell_read(i_profiler_cell_read),
  .cell_rdata(i_profiler_cell_rdata),
  .cell_write(i_profiler_cell_write),
  .cell_wdata(i_profiler_cell_wdata),
  .output_wready(i_profiler_output_wready),
  .output_wrequest(i_profiler_output_wrequest),
  .output_wdata(i_profiler_output_wdata)
);

assign i_profiler_inst_rready = control_rmx_inst_fifo_rready;
assign i_profiler_inst_opcode = inst_opcode;
assign i_profiler_inst_operand = inst_operand;

assign i_profiler_cell_addr = cell_index[0]; 
assign i_profiler_cell_read = cell_renable[0] & (~cell_stall_list[0]);
assign i_profiler_cell_rdata = cell_rdata[0];
assign i_profiler_cell_write = cell_wenable[0] & (~cell_stall_list[0]);
assign i_profiler_cell_wdata = cell_wdata[0];

assign i_profiler_output_wready = control_rmx_output_fifo_wready;
assign control_rmx_output_fifo_wrequest = i_profiler_output_wrequest;
assign control_rmx_output_fifo_wdata = i_profiler_output_wdata;
