...
if __name__ == '__main__':
  ...
  for ip_instance in xml_root.findall('ip_instance'):
    ...
    library_core_name = library_name
    if library_core_name.startswith('dca_matrix_mac'):
      library_core_name = 'dca_matrix_mac'
    ip_instance_name = ip_instance.find('name').text
    struct_name = library_core_name.title().replace('_','')+'HwInfo'
    variable_name = f'{ip_instance_name}_info'
    if library_core_name=='dca_matrix_mac':
      include_list.add('dca_matrix_hw.h')
      variable_list.append((library_core_name, ip_instance_name, struct_name, variable_name))
    elif library_core_name=='starc':
      include_list.add('starc.h')
      variable_list.append((library_core_name, ip_instance_name, struct_name, variable_name))
    elif library_core_name=='sram_xmi':
      pass
    elif library_core_name=='pact_dca':
      pass
    else:
  ...
  line_list.append('')
  line_list += [f'extern {struct_name}* {variable_name};' for library_core_name, ip_instance_name, struct_name, variable_name in variable_list]
  line_list.append('')
  ...
  line_list.append('')
  line_list += [f'{struct_name}* {variable_name};' for library_core_name, ip_instance_name, struct_name, variable_name in variable_list]
  ...
  for ip_instance in xml_root.findall('ip_instance'):
    ...
    if library_core_name=='dca_matrix_mac':
      ...
      matrix_size_para = int(get_ip_parameter_value(ip_instance, 'MATRIX_SIZE_PARA'))
      num_row = matrix_size_para % 100
      num_col = ((int)(matrix_size_para/100)) % 100
      if num_col==0:
        num_col = num_row
      member_assign_list = []
  ...
  line_list.append('{')
  for library_core_name, ip_instance_name, struct_name, variable_name in variable_list:
